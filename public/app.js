...
app.client.request = function (headers, path, method, queryStringObject, payload, callback) {
  ...
  requestUrl = (requestUrl.length > 0) ? path + '?' + requestUrl : path;
...
app.hideMessages = function () {
  if (document.querySelector('form')) {
    ...
    for (var i = 0; i < allForms.length; i++) {
      ...
      if (document.querySelector('#' + allForms[i].id + ' .formError')) {
        document.querySelector('#' + allForms[i].id + ' .formError').style.display = 'none';
      }

      // Hide the success message (if it's currently shown due to a previous error)
      if (document.querySelector('#' + allForms[i].id + ' .formSuccess')) {
        document.querySelector('#' + allForms[i].id + ' .formSuccess').style.display = 'none';
      }
    }
  }
};
        ...
        app.client.request(undefined, path, method, queryStringObject, payload, function (statusCode, responsePayload) {
          ...
          if (statusCode !== 200) {
            ...
            if (statusCode == 403) {
              ...
              document.querySelector('#' + formId + ' .formError div').innerHTML = error;
...
app.formResponseProcessor = function (formId, requestPayload, responsePayload) {
  ...
  if (formId == 'accountCreate') {
    ...
    app.client.request(undefined, 'api/tokens', 'POST', undefined, newPayload, function (newStatusCode, newResponsePayload) {
      ...
      if (newStatusCode !== 200) {
        ...
        // Set the formError field with the error text
        document.querySelector('#' + formId + ' .formError div').innerHTML = 'Sorry, an error has occured. Please try again.';
  ...
  if (formId.substring(0, 7) == 'cartAdd' || formId.substring(0, 10) == 'cartRemove') {
  ...
  if (formId == 'makeOrder') {
    app.client.request(undefined, 'api/carts', 'DELETE', undefined, newPayload, function (newStatusCode, newResponsePayload) {
      // Display an error on the form if needed
      if (newStatusCode !== 200) {
      ...
      if (statusCode == 200) {
        ...
        if (cartItems.length > 0) {
          ...
          cartItems.forEach(function (item) {
            ...
            app.client.request(undefined, 'api/products', 'GET', newQueryStringObject, undefined, function (statusCode, responsePayload) {
              if (statusCode == 200) {
                ...
                td0.innerHTML = responsePayload.productName;
                td1.innerHTML = '<form class="form-check form-check-inline" id="cartRemove' + item.productId + '" action="/api/carts" method="DELETE">' +
                  '<input type="hidden" name="_method" value="DELETE">' +
                  '<input type="hidden" name="productId" value="' + item.productId + '">' +
                  '<button type="submit" class="btn btn-sm btn-danger"><i class="fas fa-minus"></i></button>' +
                  '</form>' +
                  item.items +
                  '<form class="form-check form-check-inline" id="cartAdd' + item.productId + '" action="/api/carts" method="POST">' +
                  '<input type="hidden" name="productId" value="' + item.productId + '">' +
                  '<input type="hidden" name="items" value="1">' +
                  '<button type="submit" class="btn btn-sm btn-success"><i class="fas fa-plus"></i></button>' +
                  '</form>';
                td2.innerHTML = responsePayload.unitPrice;
              } else {
                console.log("Error trying to load product ID: ", productId);
              }
            });
          });
          setTimeout(function(){
